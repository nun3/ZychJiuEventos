...
export default function RegistrationForm({ event, athletes, categories, registeredIds }: { event: Event; athletes: Athlete[]; categories: Category[]; registeredIds: string[] }) {
                ...
                <Card key={athlete.id} className={`p-mc-16 transition-colors duration-mc-normal ${selected.includes(athlete.id) ? 'border-mc-action bg-mc-action/5' : ''}`}>
                  <div className="flex items-start gap-mc-12">
                    <input
                      ...
                      onChange={e => setSelected(ids => e.target.checked ? [...ids, athlete.id] : ids.filter(id => id !== athlete.id))}
