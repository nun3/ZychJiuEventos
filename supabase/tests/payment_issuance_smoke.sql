-- Execute after 202609060002_payment_issuance.sql. All fixtures roll back.
begin;
do $$
declare
...
begin
  ...
  -- Internal issuance RPCs must never be callable by a browser.
  if has_function_privilege('authenticated','public.claim_payment_issuance(uuid,uuid)','EXECUTE')
    or has_table_privilege('authenticated','public.payment_customers','SELECT') then
    raise exception 'FAIL: privilegio interno exposto';
  end if;
  reset role;
  insert into public.event_phases(event_id,tipo,inicio,fim)
    values(v_event_id,'pagamento',now()-interval '1 hour',now()+interval '1 day');
  insert into public.payment_customers(user_id,gateway_customer_id) values(u,'cus_smoke_'||u::text);
  set local role service_role;
  rejected := false;
  begin
    perform public.claim_payment_issuance(v_payment_id,outsider);
  exception when others then
    if sqlerrm<>'Pagamento sem permissao' then raise; end if;
    rejected := true;
  end;
  if not rejected then raise exception 'FAIL: claim de terceiro'; end if;
  claim := public.claim_payment_issuance(v_payment_id,u);
  if claim->>'kind' is distinct from 'claimed' or (claim->'input'->>'totalCents')::numeric<>24690 then
    raise exception 'FAIL: claim inicial';
  end if;
  repeated := public.claim_payment_issuance(v_payment_id,u);
  if repeated->>'kind' is distinct from 'busy' then raise exception 'FAIL: claim duplicado'; end if;
  rejected := false;
  begin
    perform public.complete_payment_issuance(v_payment_id,(claim->>'token')::uuid,'pay_smoke_'||u::text,
      claim->'input'->>'reference',1,'pix',null);
  exception when others then
    if sqlerrm<>'Cobranca divergente' then raise; end if;
    rejected := true;
  end;
  if not rejected then raise exception 'FAIL: valor divergente'; end if;
  perform public.complete_payment_issuance(v_payment_id,(claim->>'token')::uuid,'pay_smoke_'||u::text,
    claim->'input'->>'reference',246.90,'pix',null);
  repeated := public.claim_payment_issuance(v_payment_id,u);
  if repeated->>'kind' is distinct from 'issued' then raise exception 'FAIL: recuperar emitida'; end if;
  if (select count(*) from public.payment_attempts pa where pa.payment_id=v_payment_id)<>1 then
    raise exception 'FAIL: quantidade de tentativas';
  end if;
  if (select p.status from public.payments p where p.id=v_payment_id)<>'aguardando' then
    raise exception 'FAIL: emissao marcou pago';
  end if;
  if exists(select 1 from public.registrations r where r.event_id=v_event_id and r.status<>'pendente_pagamento') then
    raise exception 'FAIL: emissao efetivou inscricao';
  end if;
  reset role;
  set local role authenticated;
...
$$;
rollback;
select 'APROVADO: reserva e emissao interna, claim, duplicidade, valor, isolamento e ausencia de efetivacao prematura' as resultado;
