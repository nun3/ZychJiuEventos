...
begin
  ...
  select count(distinct registration_id), count(*)
  ...
  if target_event.status <> 'pagamento' then raise exception 'Evento fora da fase de pagamento'; end if;

  for registration_record in
    select r.*
    from public.registrations r
    where r.id = any(target_registration_ids)
    order by r.id
    for update
  loop
    locked_count := locked_count + 1;
    if registration_record.event_id <> target_event_id then
      raise exception 'Inscricoes devem pertencer ao mesmo evento';
    end if;
    if not public.manages_athlete(registration_record.athlete_id) then
      raise exception 'Inscricao sem permissao';
    end if;
    if registration_record.status <> 'pendente_pagamento' then
      raise exception 'Inscricao nao esta pendente de pagamento';
    end if;
    if exists (
      select 1
      from public.payment_registrations pr
      join public.payments p on p.id = pr.payment_id
      where pr.registration_id = registration_record.id
        and p.status in ('aguardando', 'pago')
    ) then
      raise exception 'Inscricao ja possui pagamento ativo';
    end if;
  ...
  insert into public.payments (
    id, event_id, created_by, valor_total, metodo, status, gateway, external_reference
  ) values (
    new_payment_id, target_event_id, auth.uid(), total_value, target_method,
    'aguardando', 'asaas', new_reference
  );

  insert into public.payment_registrations (payment_id, registration_id, amount)
  select new_payment_id, r.id, r.valor
  from public.registrations r
  where r.id = any(target_registration_ids);
...
revoke insert, update, delete on public.payment_registrations from authenticated, anon;
