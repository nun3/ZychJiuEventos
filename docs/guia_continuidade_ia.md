...
**Objetivo atual:** o Full Event BDD e as Sprints 8 a 13 lote 1 estão congelados. A Sprint 13 lote 2 entregou o Professor como ator cadastral (`npm run test:professor`). Não transformar Professor em `organization_role`. Não iniciar exportação nem preparação para produção. Filtro por professor, professor operacional, pesagem individual, placar, horário automático e chamada ao vivo permanecem fora.
