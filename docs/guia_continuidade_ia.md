...
**Objetivo atual:** o Full Event BDD e as Sprints 8 a 13 estão congelados. O Professor cadastral já nasce no produto (`npm run test:professor`) e não é `organization_role`. Não iniciar exportação nem preparação para produção. Filtro por professor, professor operacional, pesagem individual, placar, horário automático e chamada ao vivo permanecem fora.
